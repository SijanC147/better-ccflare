...
- **The upstream maintainer is live and proven, tracked as `SB23-1645`.** `.github/upstream-maintainer.yml` merged in PR #48. 15 gates, 13 `path_rules`, every gate mutation-tested, independent review APPROVE after three rounds. A full `dry_run=true` dispatch passed end to end against base `e6d2748f` / upstream `ea0e3320` (106 commits, 118 files, 671 KB diff): the Copilot resolver merged it, **all 15 gates passed first time with zero repair rounds**, and the reviewer approved with reasoning that engaged the real risk list. Remaining to make it run unattended: the operator steps outside the repo (install the App on this repo, confirm `TARGETS` includes it, grant Issues: write). Still unexercised: the repair path, which no run has ever entered, and the rounds-0/rounds-1 graph shapes. Read `mem:upstream_maintainer_gates` before editing that config: `protected_paths` alone protects nothing, `forbidden_patterns` reads only the upstream tree, and `*` never crosses a slash in a glob
