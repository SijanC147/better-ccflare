...
- **Ready to merge, not merged: PR #48, branch `ci/upstream-maintainer-config`, head `9b950d7e`**, tracked as `SB23-1645` — adds `.github/upstream-maintainer.yml`, the config for the GitHub App that resolves upstream-sync conflicts. 15 gates and 13 `path_rules`, every gate mutation-tested, 15/15 in a container rehearsal, the local replay run end to end against base `70b62aab` / upstream `ea0e3320` (106 commits), independent review APPROVE after three rounds, and both required checks green at the head. Remaining: (1) **sequence against `SijanC147/upstream-maintainer#34`** — the controller fetched the target branch by bare name and this repo has both a `main` branch and a `main` tag, so every run aborts in `prepare`; until #34 lands the Monday 05:00 schedule fails; (2) merge #48; (3) operator steps outside the repo: install the App, add the repo to `TARGETS`, grant Issues: write; (4) `dry_run=true` dispatch, the only way to reach the Copilot resolver and reviewer, still the one unexercised stage. Read `mem:upstream_maintainer_gates` before editing that config: `protected_paths` alone protects nothing, `forbidden_patterns` reads only the upstream tree, and `*` never crosses a slash in a glob
