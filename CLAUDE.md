...
- **In flight: PR #48, branch `ci/upstream-maintainer-config`, head `1417e44f`** — adds `.github/upstream-maintainer.yml`, the config for the GitHub App that resolves upstream-sync conflicts. Do not merge yet. Remaining, in order: (1) add gates `path-validator-log-level` (the `log.info` line in `packages/security/src/path-validator.ts` must not contain `resolvedPath`), `no-query-param-api-key` (`extract-api-key.ts` has no `searchParams`; no `searchParams.get("api_key")` under `packages/http-api/src` or `packages/proxy/src`; `STREAM_TOKEN_PATHS` still present in `auth-service.ts`), `oauth-exemption-literals` (`auth-service.ts` keeps both `/api/oauth/qwen/status/` and `/api/oauth/codex/status/`); (2) comment `version-not-bumped-by-sync` to say it fires on nearly every sync and consumes one repair round by design; (3) re-run `config-check`, the container rehearsal, and mutation-test each new gate; (4) run the local replay via the `verifying-upstream-maintainer-config` skill against `4d27cb22..upstream/main` — **the merge and policy stages have never run against real commits**; (5) correct the PR body, which overclaims "verified, not asserted" as if it covered the whole pipeline; (6) merge. Gate-authoring traps and the rehearsal profile are in `mem:upstream_maintainer_gates`
