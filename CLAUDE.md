...
- **In flight: PR #48, branch `ci/upstream-maintainer-config`, head `b772a098`**, tracked as `SB23-1645` — adds `.github/upstream-maintainer.yml`, the config for the GitHub App that resolves upstream-sync conflicts. 13 gates, all mutation-tested, 13/13 in a container rehearsal, and the local replay run end to end against base `70b62aab` / upstream `ea0e3320` (106 commits): `prepare`, `package`, `validate`. The PR body is corrected and names what is still unexercised. Remaining: (1) act on the independent review of `b772a098`; (2) **sequence against `SijanC147/upstream-maintainer#34`** — the controller fetched the target branch by bare name and this repo has both a `main` branch and a `main` tag, so every run aborted in `prepare`; until #34 lands the Monday 05:00 schedule fails; (3) merge #48; (4) operator steps outside the repo: install the App, add the repo to `TARGETS`, grant Issues: write; (5) `dry_run=true` dispatch, the only way to reach the Copilot resolver and reviewer. Everything measured about gates, `path_rules` and the replay is in `mem:upstream_maintainer_gates` — read it before editing that config, especially that `protected_paths` alone protects nothing
