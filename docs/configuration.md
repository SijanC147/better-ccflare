...
| `CCFLARE_CODEX_CACHE_DIAGNOSTICS` | Set to `1` for passive native Responses and Anthropic-to-Codex final-wire fingerprints, request/attempt lifecycle, raw terminal usage and bounded upstream diagnostic reasons. Unknown counters remain null; missing identity and size limits emit capture gaps while raw usage can still be observed. Retains at most 64 completed and 64 pending fingerprints for 30 minutes, capped at 2,048 items and 8 MiB per input. Does not change requests, routing, or cache options. Byte overlap is not token reuse | `0` | `CCFLARE_CODEX_CACHE_DIAGNOSTICS=1` |
| `CCFLARE_CODEX_CACHE_TELEMETRY_PATH` | Optional absolute path for the dedicated metadata-only diagnostic journal when diagnostics are enabled. Use a persistent private directory with one process writer. Files are private, fsynced, and rotate through four approximately 16 MiB files; startup, rotation and dropped-write counts expose capture loss. No bodies, credentials, raw IDs, or arbitrary error text are stored | - (disabled) | `CCFLARE_CODEX_CACHE_TELEMETRY_PATH=/data/cache-telemetry/events.jsonl` |
| `CCFLARE_CODEX_SYNTHETIC_COUNT_TOKENS` | Set to `0` to make Codex's synthetic `count_tokens` endpoint return a typed error instead of a character-based estimate, so gateways don't mistake the estimate for a real upstream count. Either way (estimate or typed error), the response carries `x-better-ccflare-synthetic-response: true` so a consumer can tell it didn't come from the real upstream — check this header if you'd rather detect synthetic responses generically instead of disabling the estimate outright. Default: enabled (estimate) | `1` | `CCFLARE_CODEX_SYNTHETIC_COUNT_TOKENS=0` |
