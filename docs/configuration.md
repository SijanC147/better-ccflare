...
| `CCFLARE_CODEX_CACHE_DIAGNOSTICS` | Set to `1` to observe outgoing Anthropic-to-Codex prefix stability and raw terminal cache-counter presence. Emits hashes, counts, change flags, prior-request linkage/age, cache-write and reasoning counts, and allowlisted upstream cache diagnostic types/reasons when returned. Unknown counters remain null. Does not request upstream comparisons or change requests or routing. Retains at most 64 completed and 64 pending fingerprints for 30 minutes, each capped at 2,048 items and 8 MiB input. Requests without account/session identity or over the cap are not observed. Byte overlap is not token reuse. Disable after diagnosis | `0` | `CCFLARE_CODEX_CACHE_DIAGNOSTICS=1` |
