...
| `CCFLARE_CODEX_CACHE_DIAGNOSTICS` | Set to `1` to observe outgoing Anthropic-to-Codex prefix stability and raw terminal cache-counter presence. Emits only hashes, counts, and change flags in `Codex outgoing cache diagnostics`; does not change requests or routing. Retains at most 64 completed and 64 pending fingerprints for 30 minutes, each capped at 2,048 items and 8 MiB input. Requests without account/session identity or over the cap are not observed. Byte overlap is not token reuse. Disable after diagnosis | `0` | `CCFLARE_CODEX_CACHE_DIAGNOSTICS=1` |
