...
| `CCFLARE_OVERLOAD_RETRY_ENABLED` | **Deprecated.** Predates `retry_attempts`, `retry_delay_ms` and `retry_backoff` and still overrides them when set. Set to `false` to disable retry entirely. Also switches off the in-place retry for transient upstream server errors (HTTP 500/502/503/504), which share this flag — those responses are then benched and failed over immediately, without a re-issue; the failover half is governed separately by `CCFLARE_SERVER_ERROR_RETRY_ENABLED` below. Setting the three variables in this group pins the old behavior across an upgrade; leaving them unset does not, see the two rows below | `true` | `CCFLARE_OVERLOAD_RETRY_ENABLED=false` |
| `CCFLARE_OVERLOAD_RETRY_MAX_ATTEMPTS` | **Deprecated.** Overrides `retry_attempts` when set, total attempts including the first. Integer, clamped to 1–10; an invalid or non-finite value is ignored and `retry_attempts` applies. When unset, the 529 and 5xx in-place retries fall back to `retry_attempts` instead of the old default of `2`, changing behavior on upgrade for a deployment that never set this variable | `retry_attempts` (3) | `CCFLARE_OVERLOAD_RETRY_MAX_ATTEMPTS=3` |
| `CCFLARE_OVERLOAD_RETRY_BASE_MS` | **Deprecated.** Overrides `retry_delay_ms` when set; `0` = no delay. When unset, the 529 and 5xx in-place retries fall back to `retry_delay_ms` instead of the old default of `750`, changing behavior on upgrade for a deployment that never set this variable | `retry_delay_ms` (1000) | `CCFLARE_OVERLOAD_RETRY_BASE_MS=500` |
...
| `BETTER_CCFLARE_AUTO_VACUUM` | Set to `false` to disable the unattended `PRAGMA incremental_vacuum` reclaim backstop (both the hourly retention-driven tick and the 5-minute catch-up tick that runs when the freelist grows large — see [Database Configuration](#database-configuration)). Retention deletion and payload cleanup are unaffected; only the reclaim step is skipped. **Editing the env var or the on-disk config file has no effect on an already-running process — set it and restart *before* the window starts.** (The two periodic reclaim ticks themselves pick up an in-process change on their very next run; there is just no API route or CLI flag today that changes the switch without also restarting.) Only the literal values `false` and `0` disable it; any other value (including `off`, `no`, or `disabled`) is treated as enabled. Useful during an external backup, a manual `bun run cli --compact`, or another maintenance window where you want zero automatic writer-slot contention from reclaim. SQLite only — no-op on PostgreSQL, which has no freelist/incremental-vacuum concept. Same as the `auto_vacuum_enabled` config file field; env var takes precedence | `true` | `BETTER_CCFLARE_AUTO_VACUUM=false` |
...
**Freelist reclaim.** SQLite runs in `auto_vacuum=INCREMENTAL` mode: deleted rows go to an internal freelist and are reused by new inserts, but the file only shrinks back down when free pages are explicitly returned to the OS via `PRAGMA incremental_vacuum`. Two unattended background ticks drive this:

- An **hourly** tick, run right after the retention cleanup that deletes expired requests/payloads, reclaiming up to ~1 GiB of freed pages.
- A **5-minute catch-up** tick that only does anything when the freelist has grown to 10% or more of the database file's total pages — it reclaims a smaller amount per tick (~256 MiB) but, at 12 ticks/hour, adds meaningfully more throughput than the hourly tick alone can provide on a database seeing a sustained high delete rate. It also backs off for a round whenever the async DB writer hasn't finished draining its own queue — a best-effort backpressure check made once before the tick dispatches, not a running guarantee — reducing the chance of competing with in-flight request logging for SQLite's single writer slot; writes arriving after that check, while the tick's own chunked reclaim is still in progress, can still contend for it.

Both ticks are bounded, chunked, and yield between chunks so a large reclaim never holds up proxy requests for long. Set `BETTER_CCFLARE_AUTO_VACUUM=false` and restart to disable both entirely. Editing the env var or the on-disk config file has no effect on an already-running process — the on-disk file is read only once, at startup; the env var is actually re-read live on every tick, but nothing in this codebase mutates `process.env` after startup, and an external edit can't reach an already-running process's environment either way, so in practice it behaves as fixed until restart — and there is no API route or CLI flag today that flips the switch in-process, so either way it must be set (and the process restarted) *before* the maintenance window it's meant to cover, not during it (see the environment variable table above for the exact accepted values). Retention deletion and payload cleanup keep running either way, only the reclaim step is skipped; a still-`auto_vacuum=NONE` file's one-time migration VACUUM (the blocking step that runs once before the HTTP listener binds on an upgraded install) is deferred the same way while the switch is off, so it never contends with a maintenance window either. No-op on PostgreSQL, which has no freelist or incremental-vacuum concept.
