...
export function parseCodexUsageHeaders(
	...
	//
	// The key is omitted rather than set to null. An earlier fork revision kept
	// both keys present because `usage-throttling.ts` gated its Codex branch on
	// `"five_hour" in data && "seven_day" in data`; upstream has since relaxed
	// that gate to `||`, so a weekly-only payload still reaches the throttle.
