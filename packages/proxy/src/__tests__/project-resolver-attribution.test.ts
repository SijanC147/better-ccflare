/**
 * Phase 3.A — Project attribution happy-path tests (fork feature).
 *
 * Renamed from project-attribution.test.ts during the 2026-08 upstream sync:
 * upstream independently added packages/proxy/src/project-attribution.ts and a
 * test file of the same name. The two are complementary, not duplicates —
 * upstream extracts a sanitized project NAME from headers/system-prompt, while
 * this covers the fork's ResolverSnapshot, which maps a filesystem PATH to a
 * registered project id + worktree. Both suites are kept.
 *
