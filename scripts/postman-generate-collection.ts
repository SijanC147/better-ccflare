...
// The same endpoints as Postman AI requests (`$kind: llm-request`), in their
// own collection. The URLs and model are literals, copied from the shape of
// Sean's working "Chat Requests" request, because variable substitution in an
// AI request's base URL and model is not established.
const AI_COLLECTION = "better-ccflare AI requests";
const AI_OUT = join(ROOT, "postman", "collections", AI_COLLECTION);
const AI_BASE = "http://localhost:8080/v1";
const AI_MODEL = "claude-opus-5-5";
/** Fixed so regeneration is byte-identical. */
const AI_TIMESTAMP = 1790208000000;

/** A UUID-shaped id derived from `seed`, stable across runs. */
...
function aiRequestYaml(r: AiRequestSpec): string {
	const prompt = (kind: string, value: string) => [
		`  - id: ${stableId(`${r.name}:${kind}`)}`,
		`    value: ${q(value)}`,
		`    timestamp: ${AI_TIMESTAMP}`,
		"    active: true",
		"    contentType: text",
	];
	const lines = ["$kind: llm-request"];
	if (stem(r.name) !== r.name) lines.push(`name: ${q(r.name)}`);
	lines.push(
		`description: ${block(r.description, 2)}`,
		`url: ${q(r.url)}`,
		"config:",
		`  model: ${q(AI_MODEL)}`,
		"  provider: openai-compatible",
		"settings:",
		`  streamResponse: ${r.stream}`,
		"  maxToken: 256",
	);
	if (r.system) lines.push("systemPrompts:", ...prompt("system", r.system));
	lines.push(
		"userPrompts:",
		...prompt("user", "Reply with the single word: pong"),
		'mcpConfig: ""',
		`order: ${r.order}`,
	);
	return `${lines.join("\n")}\n`;
...
const GATEWAY_NOTE =
	"Needs a gateway named `no-oauth`. Create it once with the `PUT /api/openai-gateways/:name` request in the better-ccflare API collection (name `no-oauth`, body `{\"exclude_providers\": [\"anthropic-oauth\"]}`), or change the name in the URL to one from `GET /api/openai-gateways`.";

const aiRequests: AiRequestSpec[] = [
	{
		name: "Chat completion",
		url: AI_BASE,
		order: 1000,
		...
		description:
			"`POST /v1/chat/completions` through the default gateway, with a system prompt. Spends real account quota on 8080.",
	},
	{
		name: "Chat completion (stream)",
		url: AI_BASE,
		...
		stream: true,
		description:
			"`POST /v1/chat/completions` with `stream: true`. Spends real account quota on 8080.",
	},
	{
		name: "Named gateway no-oauth",
		url: `${AI_BASE}/gateways/no-oauth`,
		order: 3000,
		stream: false,
		description: `The \`no-oauth\` gateway excludes Claude OAuth accounts. ${GATEWAY_NOTE}`,
	},
	{
		name: "Named gateway no-oauth (stream)",
		url: `${AI_BASE}/gateways/no-oauth`,
		order: 4000,
		stream: true,
		description: `Streamed through the \`no-oauth\` gateway. ${GATEWAY_NOTE}`,
	},
];

rmSync(AI_OUT, { recursive: true, force: true });
mkdirSync(join(AI_OUT, ".resources"), { recursive: true });
writeFileSync(
	join(AI_OUT, ".resources", "definition.yaml"),
	[
		"$kind: collection",
		`description: ${block(
			[
				"Generated by `scripts/postman-generate-collection.ts`. Edit the script, not these files.",
				"better-ccflare's OpenAI-compatible endpoint as Postman AI requests. Every request here routes to real accounts on the production server on 8080.",
			].join("\n\n"),
			2,
		)}`,
		"auth:",
		`  - id: ${stableId("ai-collection-auth")}`,
		"    type: apikey",
		"    name: API Key",
		"    credentials:",
		"      key: x-api-key",
...
for (const r of aiRequests)
	writeFileSync(join(AI_OUT, `${stem(r.name)}.request.yaml`), aiRequestYaml(r));

const written = API_ROUTES.length + smoke.length + gateway.length;
console.log(`wrote ${written} requests to ${OUT}`);
console.log(`wrote ${aiRequests.length} AI requests to ${AI_OUT}`);
