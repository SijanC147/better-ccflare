...
const openAIErrorTest = (code: number, errorCode: string) =>
	[
		statusTest(code),
...
const smoke: RequestSpec[] = [
	{
		...
		tests: [
			statusTest(200),
			'pm.test("lists every catalog route", () => {',
			"  const body = pm.response.json();",
	...
	},
	{
		...
		tests: [
			statusTest(200),
			'pm.test("JSON body", () => pm.expect(pm.response.json()).to.be.an("object"));',
		].join("\n"),
	},
	{
		...
		order: 3000,
		description:
			"Refused by the request translator before routing, so it reaches no account.",
		headers: JSON_HEADER,
		body: chatBody({ n: 2 }),
		tests: openAIErrorTest(400, "unsupported_value"),
	},
	{
		name: "Unknown gateway is 404",
		method: "POST",
		url: "{{baseUrl}}/v1/gateways/postman-smoke-no-such-gateway/chat/completions",
		order: 4000,
		description:
			"An unknown gateway name must refuse, never fall through to the unrestricted route. Reaches no account.",
		headers: JSON_HEADER,
		body: chatBody(),
	...
	definitionYaml(
		{
			...
			description: [
				...
				"Only `Smoke` is safe to run as a batch against 8080. `OpenAI gateway` spends real account quota, and the catalog folders include mutating and dangerous routes.",
			].join("\n\n"),
		},
		[
			"variables:",
...
writeFolder(
	join(OUT, "Smoke"),
	definitionYaml({
		description: "Checks that reach no account. Safe to run against 8080.",
		order: 1000,
	}),
	smoke,
);
